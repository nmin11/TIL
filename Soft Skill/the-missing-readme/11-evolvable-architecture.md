...
- API는 영구적인 데이터에 비하면 수명이 짧은 편
- 데이터는 애플리케이션의 변화와 함께 반드시 진화해야 함
- 데이터의 진화가 포함하는 것들
  - 컬럼 추가 및 삭제와 같은 간단한 스키마 변경
  - 새 스키마로 레코드를 새로 작성하는 작업
  - 손상 데이터 복구
  - 새 비즈니스 로직을 위한 재작성
  - DB 간의 마이그레이션
...
- 여러 애플리케이션이 공유 DB를 사용하지 않도록 하자
- DB를 애플리케이션 별로 격리하면 유연성을 갖출 수 있음

### schema

- 스키마를 사용하면 모호성이 줄어들고 복잡도가 낮아짐
- 스키마가 없으면 데이터를 명확히 이해할 수 없어서 변경이 더 어려워질 수 있음
- 하지만 명시적 스키마가 데이터 변경이 어렵도록 만드는 것도 사실
- 신속한 개발 및 변경이 필요하다면 스키마 없는 데이터를 사용해볼 수 있을 것
- 묵시적 스키마를 사용하다가 명시적 스키마로 데이터를 마이그레이션하는 것도 유용한 방법

### automate schema migration

- 직접 DDL(Database Description Language) 명령어를 이용해서 스키마를 변경하는 것은 위험
- DB 스키마 관리 도구를 활용하자
  - 스키마 전체 히스토리 추적 기능
  - 스키마 버전 마이그레이션 기능
  - 롤백 기능
  - Liquibase가 유명
- DDL 변경사항을 파일로 만들어 추적하면 스키마 변경의 흐름을 이해하는 데 도움이 됨
- ORM(Object-Resource Mapper)도 스키마 마이그레이션 관리자를 제공

※ 정교한 DB 운영 도구들: `gh-ost` `pt-online-schema` `Skeema` `Shift`

### schema compatibility

- **스키마 호환성 검사를 하고 데이터 제품을 이용해서 내부 및 외부 스키마를 분리하자!**
- DB를 완전히 캡슐화하는 것은 이상적이긴 하지만 현실적으로 그럴 수가 없음
  - 데이터가 데이터 웨어하우스로 내보내지는 경우가 많기 때문

데이터 웨어하우스

- 분석과 보고서 목적의 DB
- 프로덕션 DB로부터 데이터 웨어하우스로 로드

ETL 데이터 파이프라인

- ETL: Extract, Transform, Load (추출, 변형, 적재)
- 애플리케이션 DB로부터 데이터를 추출하고 변형을 가한 후 데이터 웨어하우스에 적재
- DB 스키마에 크게 의존
  - 컬럼 하나를 지워도 데이터 파이프라인이 멈출 수 있음

CDC(Change Data Capture)

- 삽입, 갱신, 삭제 작업을 다운스트림 컨슈머를 위한 메시지로 변환하는 이벤트 기반 아키텍처
- 잘 드러나지 않는 API이기 때문에 비호환적인 스키마 변경 문제에 취약
