...
<!-- 이미지 추천: seq2seq 인코더-디코더 아키텍처 다이어그램.
     왼쪽 인코더(RNN)가 입력 시퀀스를 순차 처리해 하나의 은닉 상태 벡터를 만들고,
     오른쪽 디코더(RNN)가 이를 받아 출력 토큰을 순차 생성하는 그림.
     "Sequence to Sequence Learning" (Sutskever et al., 2014) 논문 Figure 1 참고. -->
...
- seq2seq는 인코더와 디코더로 구성됨
  ...
  - 인코더는 입력 토큰을 순차적으로 처리하여 입력을 표현하는 최종 은닉 상태(hidden state) 출력
...
<!-- 이미지 추천: "Attention Is All You Need" 논문의 트랜스포머 아키텍처 전체 그림 (Figure 1).
     왼쪽 인코더 스택과 오른쪽 디코더 스택이 Multi-Head Attention, Feed Forward 레이어로 구성된 모습.
     또는 트랜스포머를 직관적으로 설명한 "The Illustrated Transformer" (Jay Alammar 블로그) 의 그림들 참고. -->
...
- 트랜스포머 기반 언어 모델은 모든 트랜스포머 블록 앞뒤에 모듈을 갖추고 있음
  ...
  - **블록 이후의 출력 레이어**
    - 모델의 출력 벡터를 모델 출력으로 샘플링하는 데 사용되는 토큰 확률로 매핑
...
<!-- 이미지 추천: 온도 값에 따른 softmax 확률 분포 변화 그래프.
     같은 로짓 벡터에 temperature=0.1(뾰족하게 하나 집중), temperature=1.0(완만), temperature=2.0(평평하게 분산)을
     나란히 bar chart나 line chart로 보여주는 그림. -->
...
<!-- 이미지 추천: top-k와 top-p 샘플링 비교 다이어그램.
     확률 분포 bar chart에서 top-k는 상위 k개를 고정 컷오프로 자르고,
     top-p는 누적 확률이 p를 넘는 지점에서 동적으로 자르는 모습을 나란히 표시.
     Hugging Face 블로그 "How to generate text" 의 그림 참고. -->
