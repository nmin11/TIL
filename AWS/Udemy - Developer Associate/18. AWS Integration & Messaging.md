...
- Producers
  - 여러 형태의 생산자가 Kinesis Data Streams에 데이터를 보냄
  - producer 유형은 application, client, SDK, KPL(Kinesis, Producer Library), Kinesis Agent 등 다양
- Producer Record
  - producers에 의해 생성됨
  - 기본적으로 2가지로 구성됨
    - Partition Key: record가 어떤 shard로 이동할지 정의
    - Data Blob (최대 1MB): 전송할 값 자체
  - producer가 Kinesis Data Streams로 데이터를 보낼 때는 shard 당 1MB/sec 혹은 1000 msg/sec(MPS) 로 전송됨
- Consumers
  - consumer 유형은 Apps(KCL, SDK), Lambda, Kinesis Data Firehose, Kinesis Data Analytics 등 다양
- Consumer Record
  - 3가지로 구성됨
    - Partition Key
    - Sequence no: record가 있었던 shard 위치
    - Data blob
  - 다양한 소비 모드
    - 처리량을 공유할 때는 모든 consumer에게 shard 당 2MB/sec
    - 향상된 consumer 모드를 활성화한 경우 한 consumer에게 shard 당 2MB/sec
