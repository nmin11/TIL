...
- 버켓의 특정 prefix는 초당 최소 3,500번의 PUT/COPY/POST/DELETE 요청과 5,500번의 GET/HEAD 요청 처리 가능
  - 만약 4개의 prefix에 읽기 작업을 균일하게 분산시키면 초당 22,000번의 요청 처리 가능
- 버켓 내 prefix 개수에는 제한이 없음
...
- edge location에서는 데이터를 target region의 S3 버켓으로 보내줌
...
- 모든 이벤트에 대한 알림이 전달되도록 하기 위해서, 버켓의 버저닝을 활성화해야 함
