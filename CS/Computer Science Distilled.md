...
- 해커들은 disassembly 도구를 이용해서 이진 코드를 해석할 수 있음
  ...
  - 해커들은 Windows, Photoshop, GTA 등 사용권이 제한된 프로그램에서 이진 코드를 검사해서<br>사용권 검사 코드를 찾아낸 뒤, 해당 부분에 JUMP 명령을 배치한 불법 복제 소프트웨어를 만들 수 있는 것
  - 정부의 비밀 첩보 기관에서 만든 Stuxnet은 이란의 지하 핵융합로 제어 컴퓨터에 침입해서 핵 개발 계획을 방해하기도 했음

<br>

## 효율적인 정보 저장을 위한 메모리 계층 구조

- CPU 레지스터 용량은 대개 1천 바이트 미만
- 그러므로 CPU 레지스터는 끊임없이 RAM과 데이터를 주고받아야 함
- 따라서 메모리에 데이터를 읽고 쓰는 데 걸리는 시간은 컴퓨터의 성능에 직접적인 영향을 미침

### 프로세서-메모리 격차

- CPU의 속도는 최근 '지수적으로' 증가했음
- 메모리의 속도 또한 증가해왔지만 CPU에 비해 증가율이 '훨씬' 낮음
- 이러한 CPU와 메모리의 성능 차이를 **프로세서-메모리 격차**라고 함
- 이 격차가 벌어질수록 메모리에 효율적으로 접근하는 것이 더욱 중요해짐
- 컴퓨터과학자들은 RAM의 연산 횟수를 줄이는 방법을 연구하기 시작했음

### 시간·공간 인접 주소

- RAM 접근 횟수 최소화를 연구하던 컴퓨터과학자들이 깨달은 2가지 중요한 사실
  - temporal locality : 어떤 메모리 주소에 접근한 경우, 잠시 후 같은 주소에 재접근할 가능성 높음
  - spatial locality : 어떤 메모리 주소에 접근한 경우, 잠시 후 인접 주소에 접근할 가능성 높음
- 이에 따라 접근 가능성이 높은 메모리 주소들을 예측해서 CPU 레지스터에 저장해 두는 방식이 고안되기 시작함

### 1차 캐시

- 레지스터의 용량은 늘리지 못하더라도, CPU와 통합된 극도로 빠른 보조 메모리를 만드는 것은 가능
- 이를 **L1 cache (1차 캐시)** 라고 함
- 1차 캐시의 데이터를 레지스터로 옮기는 것은 레지스터 자체 데이터에 접근하는 것보다 아주 조금 느릴 뿐
  - 1차 캐시의 데이터를 레지스터에 옮기는 데에 약 10 CPU 주기밖에 걸리지 않음
  - 10 CPU 주기는 RAM에서 가져올 때보다 100배 정도는 빠른 것

### 2차 캐시

- 1차 캐시의 속도를 건드리지 않고 용량을 늘리는 게 또 어려움
  - 전기 신호의 속력은 일정하므로, 캐시의 용량(물리적 공간)이 커질수록 접근 속도는 느려짐
  - 1차 캐시 용량이 약 50KB를 넘어가면 용량 증설 비용이 매우 높아짐
- 1차 캐시를 늘리는 것보다 효율적인 방법이 **L2 cache (2차 캐시)** 를 사용하는 것
- 2차 캐시는 속도가 좀 더 느려도 되므로 1차 캐시보다 훨씬 큰 용량으로 만들 수 있음
- 오늘날의 CPU에는 200KB 정도의 2차 캐시가 탑재됨
- 2차 캐시의 데이터를 CPU 레지스터로 옮기는 데 약 100 CPU 주기 소요
- 접근 가능성이 가장 높은 주소는 1차 캐시에 복사하고, 그 다음으로 높은 주소를 2차 캐시에 복사
- 이렇게 설계해두면 CPU는 접근하려는 메모리를 1차 캐시 → 2차 캐시 → RAM 순으로 찾게 됨
- 상당수의 CPU 제조사가 3차 캐시를 탑재한 프로세서를 출시하고 있음
  - 3차 캐시는 2차 캐시에 비해 느리지만 RAM에 비해 빠름
- CPU에서 캐시는 무척 중요하기 때문에 CPU 칩에서 물리적인 공간 대부분을 이들이 차지함
- 2차 캐시까지 발전하게 되면서 CPU가 RAM에 직접 접근하는 메모리 요청은 10% 미만으로 줄었음!
- 앞으로 컴퓨터를 살 때 CPU의 1차·2차·3차 캐시 용량을 꼭 따져보자!
  - CPU의 클락이 좀 낮더라도 캐시 용량이 큰 게 나은 경우가 많음

### 1차 메모리와 2차 메모리

- RAM을 비롯한 메모리 계층 구조에서 한 단계 더 내려가 하드 디스크를 사용할 수도 있음
- 하지만 하드 디스크와 RAM 사이에 데이터를 옮기려면 약 1백만 CPU 주기 소요
  - CPU가 RAM에 접근하는 데 필요한 시간은 약 1천 CPU 주기
- 이런 속도와 용량 차이를 반영하여 흔히 RAM 메모리를 **1차 메모리**라 하고<br>하드 디스크에 저장된 프로그램 및 데이터를 **2차 메모리**라 부름
- CPU는 2차 메모리에 직접 접근하지 못하고 1차 메모리로 먼저 복사되어야 함
- 사실, 운영체제도 컴퓨터가 부팅될 때마다 디스크에서 RAM으로 복사된 후에 실행되는 것!

### thrashing mode

- 컴퓨터가 디스크에서 RAM으로 데이터를 끝없이 읽어들이는 상태를 thrashing mode라고 함
- 서버가 thrashing mode에 빠지면 서버 전체가 다운될 수도 있음
- 이처럼 RAM 부족은 서버에 장애를 일으키는 가장 잦은 원인
