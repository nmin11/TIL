...
길이가 m, n이고 오름차순으로 정렬되어 있는 자연수 배열들을 입력받아 전체 요소 중 k번째 요소를 리턴하라.

</br>

## 추가 사항

- arr1.length + arr2.length <= 1000000
- 합친 배열 arr의 k번째 요소는 arr[k - 1]을 뜻함
- 단순히 처음부터 끝까지 찾아보는 방법(O(n)) 대신 이진 탐색을 활용한 방법(O(log n))을 활용할 것
